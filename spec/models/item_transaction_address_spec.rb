...
RSpec.describe ItemTransactionAddress, type: :model do
  ...
  context '内容に問題がある場合' do
    it 'user_idが空だと保存できないこと' do
      @item_transaction_address.user_id = ''
      @item_transaction_address.valid?
      expect(@item_transaction_address.errors.full_messages).to include("User can't be blank")
    end
    it 'item_idが空だと保存できないこと' do
      @item_transaction_address.item_id = ''
      @item_transaction_address.valid?
      expect(@item_transaction_address.errors.full_messages).to include("Item can't be blank")
    end
